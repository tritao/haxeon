...
class HxiCallMain {
	static function main():Void {
		...
		mainSource = StringTools.replace(mainSource, "var structure = ",
			"var summed = Fixture.addPoints(start, end); var padded = Fixture.makePadded(40); var byValue = summed.get_x() == 30 && summed.get_y() == 32 && Fixture.checkBoxValue(box) == 42 && Fixture.checkArraysValue(arrays) == 42 && padded.get_tag() == 2 && padded.get_value() == 40; var structure = byValue && arrayFields && ");
