...
class WasmBackendMain {
	static function main():Void {
		...
		var stringOps = compile("function main():Int { var value = \"HaXeOn\"; return value.toLowerCase() == \"haxeon\" && value.toUpperCase() == \"HAXEON\" && value.indexOf(\"Xe\") == 2 && value.substring(2, 4) == \"Xe\" && value.charAt(2) == \"X\" && String.fromCharCode(65) == \"A\" ? 42 : 0; }");
	...
	static function compileGcExceptionProgram():haxe.io.Bytes {
		var source = File.getContent("tests/programs/wasm-gc-exceptions.hx"),
			bytes = new WasmBackend().compile(Frontend.compile(source), {target: WasmGc, debugNames: true}).bytes;
		for (forbidden in [
			"__haxeon_alloc",
			"__haxeon_gc_mark",
