...
class HxiFlagsMain {
	static function main():Void {
		var source = 'interface flag_widths @target("x86_64-linux-gnu") @library("flag_widths") { '
			...
			+ 'flags WideFlags : u64 { NONE = 0; FIRST = 1; LOW = 1 << 31; HIGH = 1 << 63; BOTH = (1 << 63) | 1; } '
		...
		switch model.declarations[3] {
			case Enumeration("WideFlags", _, true, values, _):
				expect(Int64.compare(values[3].value, Int64.parseString("-9223372036854775808")) == 0,
		...
		expect(projection.indexOf("enum abstract ByteFlags(Int)") >= 0
			...
			&& projection.indexOf("haxe.Int64.make(0, -2147483648)") >= 0
