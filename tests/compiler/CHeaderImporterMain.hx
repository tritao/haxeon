...
class CHeaderImporterMain {
	static function main():Void {
		...
		expect(first.indexOf("title: ptr<const<i8>> @offset(8)") >= 0, "pointer field offset should be preserved");
		expect(first.indexOf("extern fn sample_error() -> ptr<const<i8>>") >= 0, "pointer results should import");
