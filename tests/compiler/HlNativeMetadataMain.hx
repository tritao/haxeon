...
class HlNativeMetadataMain {
	static function main():Void {
		...
		compiler.update("HlNativeMetadataAdapter.hx",
			...
			+
			'import runtime.hashlink.HlFunctionVersionTable; import runtime.hashlink.HlHotReloadState; import runtime.hashlink.HlMetadataGeneration; import runtime.hashlink.HlNativeModule; import runtime.hashlink.HlRuntimeJitBackend.HlRuntimeModuleHandle; import runtime.hashlink.HlRuntimeModuleKernel.NativeHlRuntimeModuleKernel; import runtime.memory.GcHandle; import runtime.memory.RawPtr; '
			+ 'import compiler.hl.persistence.HlRuntimeIdentity; '
			+
			'class BlockingUnloadKernel extends NativeHlRuntimeModuleKernel { public var allowUnload:Bool = false; public function new() {} override public function unload(module:HlRuntimeModuleHandle):Bool return allowUnload && new NativeHlRuntimeModuleKernel().unload(module); } '
			+
			...
			+
			'var failedInitializerCode = new HlCode(); failedInitializerCode.strings = ["initialization failed"]; failedInitializerCode.types = [Simple(HlType.Bytes), Simple(HlType.Dyn), Simple(HlType.Void), Function([], 2)]; failedInitializerCode.functions = [new compiler.hl.HlFunction(3, 0, [0, 1], [LoadString(0, 0), ToDyn(1, 0), Throw(1)])]; failedInitializerCode.entryPoint = 0; var failedInitializerIdentity = HlRuntimeIdentity.encode(haxe.io.Bytes.alloc(16), 1, ["__init" => 0], ["__init" => 303]), failedInitializerRejected = false; try { HlNativeModuleLoader.loadRuntime(HlWriter.encode(failedInitializerCode), failedInitializerIdentity); } catch (error:Dynamic) failedInitializerRejected = true; var blockedKernel = new BlockingUnloadKernel(), failedRetirementRejected = false; try { HlNativeModuleLoader.loadRuntime(HlWriter.encode(failedInitializerCode), failedInitializerIdentity, blockedKernel); } catch (error:Dynamic) failedRetirementRejected = true; var failedRetirementQueuedCount = HlNativeModuleLoader.failedRetirementCount(), failedRetirementQueued = failedRetirementQueuedCount == 1; blockedKernel.allowUnload = true; var failedRetirementReclaimed = HlNativeModuleLoader.retryFailedRetirements() == 1 && HlNativeModuleLoader.failedRetirementCount() == 0; initializerRejected = initializerRejected && failedInitializerRejected && failedRetirementRejected && failedRetirementQueued && failedRetirementReclaimed; '
			+
