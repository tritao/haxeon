...
class HlNativeMetadataMain {
	static function main():Void {
		...
		compiler.update("HlNativeMetadataAdapter.hx",
			...
			+
			'&& generatedValue == 8 && generatedUnloaded && registryFirstLoaded && registrySecondLoaded && runtimeRegistry.generation == 2 && registryBlocked && registryReclaimed && registryValue == 8 && registryDisposed && loadedValue == 8 && loadedModuleUnloaded && complexInitialized && complexUnloaded && layoutInitialized && layoutUnloaded && externalMalformedRejected && externalUnknownIdentityRejected && externalRelocationRejected && externalValue == 8 && externalPatchedValue == 42 && externalTransactionCommitted && externalStaleRejected && externalRolledBack && externalLoaded.revision == 2 && externalPatchRejected && externalIdentityRejected && externalCallRejected && externalUnloaded && initializerRejected && patchRejected && patchFirst && patchSecond && patchValue == 42 && patchValueAgain == 43 && patchesUnloaded '
