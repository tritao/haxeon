...
class TestCatalog {
	...
	public static function executableCases():Array<ExecutableCase> {
		return [
			...
			hxmlCase("runtime", "runtime-patch-transaction", "tests/hxml/runtime-patch-transaction-test.hxml", "runtime-patch-transaction-test.hl", 0,
				"host HLP transactions stage, roll back, and commit exactly once"),
	...
	static function validateProgramManifest(root:String, programs:Array<ProgramCase>):Void {
		...
		// Unlisted Wasm sources are exercised by backend-specific runtime scripts.
		for (file in FileSystem.readDirectory(directory)) {
			if (!StringTools.endsWith(file, ".hx")
				|| StringTools.startsWith(file, "utest-")
				|| (StringTools.startsWith(file, "wasm-") && !expected.exists(file)))
				continue;
