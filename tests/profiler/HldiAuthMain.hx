...
class HldiAuthMain {
	...
	static function main():Void {
		...
		if (port == null)
			throw "Usage: hldi-auth-test.hl PORT TOKEN";
		if (!rejected(port, null))
			throw "missing HLDI token was accepted";
		if (!rejected(port, args[1] + "-wrong"))
			throw "incorrect HLDI token was accepted";
		var client = new HldiClient("127.0.0.1", port, 3.0, args[1]);
		if (client.hello.capabilities & HldiClient.CAP_AUTH_REQUIRED == 0)
			throw "endpoint did not advertise required authentication";
		client.status();
		client.close();
