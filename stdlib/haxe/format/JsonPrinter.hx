...
class JsonPrinter {
	...
	function write(key:Dynamic, original:Dynamic):Void {
		...
		switch jsonValueKind(value) {
			case 0:
				output.add("null");
			case 1:
				quote(cast(value, String));
			case 2:
				output.add(cast(value, Bool) ? "true" : "false");
			case 3:
			...
			case 5:
				writeArray(value);
			case 6:
				throw "Cannot encode function as JSON";
			default:
				writeObject(value);
		}
	}

	function writeArray(value:Dynamic):Void {
		enter(value);
		output.add("[");
		depth++;
		#if wasm
	...
	function enter(value:Dynamic):Void {
		for (active in stack)
			if (active == value)
				throw "Cyclic value cannot be encoded as JSON";
		stack.push(value);
	}
	...
	function quote(value:String):Void {
		...
		for (index in 0...value.length) {
			var code = value.charCodeAt(index),
				escaped:Null<String> = switch code {
					case 34: '\\"';
					case 92: '\\\\';
					case 8: '\\b';
					case 9: '\\t';
