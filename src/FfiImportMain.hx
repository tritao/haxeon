...
class FfiImportMain {
	static function main():Void {
		var args = Sys.args(), target = "", output = "", includes:Array<String> = [], paths:Array<String> = [];
		for (arg in args)
		...
		if (target.length == 0 || output.length == 0 || paths.length != 1)
			throw "Usage: haxeon-ffi-import --target=<triple> --output=<file> [--include=<dir>] <header>";
		File.saveContent(output, CHeaderImporter.importHeader(paths[0], target, includes));
