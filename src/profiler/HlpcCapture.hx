...
class HlpcCapture {
	...
	public function new(path:String, processId:Int, sampleRate:Int) {
		...
		put16(header, 4, 1); put16(header, 6, 24);
		put32(header, 12, processId); put32(header, 16, sampleRate);
		output.write(header);
	}
	...
	public function samples(requested:Int64, next:Int64, dropped:Int64, bytes:Bytes):Void {
		...
		put64(payload, 0, requested); put64(payload, 8, next); put64(payload, 16, dropped);
	...
	public function close(cursor:Int64, dropped:Int64):Void {
		...
		put64(payload, 0, cursor); put64(payload, 8, dropped);
		record(3, payload);
		closed = true;
		output.close();
	}
	...
	function record(type:Int, payload:Bytes):Void {
		...
		put32(header, 0, type); put32(header, 4, payload.length);
		put64(header, 8, Int64.fromFloat((Timer.stamp() - started) * 1000000000.0));
		output.write(header); output.write(payload); output.flush();
	}

	static function put16(bytes:Bytes, at:Int, value:Int):Void {
		bytes.set(at, value); bytes.set(at + 1, value >>> 8);
	}
	static function put32(bytes:Bytes, at:Int, value:Int):Void {
		for (index in 0...4) bytes.set(at + index, value >>> (index * 8));
	}
	static function put64(bytes:Bytes, at:Int, value:Int64):Void {
		put32(bytes, at, value.low); put32(bytes, at + 4, value.high);
	}
}
