...
class WasmRepresentationSet {
	...
	public static function gc(gc:WasmGcContext, representation:WasmGcRepresentation, interop:WasmGcInterop):WasmRepresentationSet
		return new WasmRepresentationSet(representation, representation, representation, interop,
			function(context) return gcForFunction(gc, representation, interop, context));

	static function gcForFunction(gc:WasmGcContext, representation:WasmGcRepresentation, interop:WasmGcInterop,
			context:WasmFunctionRepresentationContext):WasmRepresentationSet {
		var functionContext = new WasmGcFunctionContext(gc, context.irFunction, context.exceptionTag, context.allocateLocal),
			functionRepresentation = representation.forFunctionContext(functionContext),
			functionInterop = interop.forFunctionContext(functionContext);
		return new WasmRepresentationSet(functionRepresentation, functionRepresentation, functionRepresentation, functionInterop,
			function(context) return gcForFunction(gc, representation, interop, context));
	}
}
