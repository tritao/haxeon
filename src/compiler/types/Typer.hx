...
class Typer {
    ...
    public static function type(program:AstProgram):TypedProgram return new Typer(null).typeProgram(program, null);
    public static function typeSelected(program:AstProgram, selected:Map<String, Bool>,?externals:Map<String,{arguments:Array<CompilerType>,result:CompilerType}>):TypedProgram return new Typer(externals).typeProgram(program, selected);
    function new(externals) this.externals=externals==null?[]:externals;
    ...
    function typeProgram(program:AstProgram, selected:Null<Map<String, Bool>>):TypedProgram {
        for (fn in program.functions) {
            ...
            if(externals.exists(fn.name))fail("E1000",'Function "${fn.name}" conflicts with a registered native',fn.span);
    ...
    function typeExpression(expression:AstExpression, scope:Scope):TypedExpression return switch expression {
        ...
        case Call(name, arguments, span):
            ...
            var external=externals.get(name),expectedArguments=signature==null?(external==null?null:external.arguments):[for(argument in signature.arguments)lowerType(argument.type)],result=signature==null?(external==null?null:external.result):lowerType(signature.result);
            if (expectedArguments == null) fail("E1007", 'Unknown function "$name"', span);
            if (arguments.length != expectedArguments.length) fail("E1008", 'Function "$name" expects ${expectedArguments.length} arguments, got ${arguments.length}', span);
            var typed = [for (argument in arguments) typeExpression(argument, scope)];
            for (i in 0...typed.length) if (typed[i].type != expectedArguments[i]) fail("E1009", 'Argument ${i + 1} to "$name" has the wrong type', typed[i].span);
    ...
    function numeric(a,b,scope,operation,span):TypedExpression {var left=typeExpression(a,scope),right=typeExpression(b,scope);if(left.type!=right.type||(left.type!=TInt&&left.type!=TFloat))fail("E1010","Arithmetic requires matching Int or Float operands",span);return new TypedExpression(operation==2?TMul(left,right):TDiv(left,right),left.type,span);}
