...
class HxiNativeRecordEmitter {
		...
		for (declaration in model.declarations)
			switch declaration {
				case Structure(name, _, _, fields, _):
					emitStructure(output, name, fields, declarations, structureNames, typePrefix, fieldNames);
				case _:
			}
	...
	static function emitStructure(output:StringBuf, name:String, fields:Array<HxiField>, declarations:Map<String, HxiDeclaration>,
			structureNames:Map<String, String>, typePrefix:String, fieldNames:Null<Map<String, String>>):Void {
		...
		if (unionFields.length > 0) {
			...
			for (field in unionFields)
				emitField(output, name, field, declarations, structureNames, fieldNames);
			output.add('}\n\n');
		}
	...
	static function emitField(output:StringBuf, owner:String, field:HxiField, declarations:Map<String, HxiDeclaration>, structureNames:Map<String, String>,
		...
		output.add('\tpublic var $name:${renderType(type, declarations, structureNames, [])};\n');
	}

	static function renderType(type:HxiType, declarations:Map<String, HxiDeclaration>, structureNames:Map<String, String>, resolving:Map<String, Bool>):String {
		return switch type {
			case Const(element) | Nullable(element): renderType(element, declarations, structureNames, resolving);
			case Pointer(element): 'RawPtr<${renderPointee(element, declarations, structureNames, resolving)}>';
			...
			case Named(name):
				...
				var result = switch declarations.get(name) {
					...
					case Enumeration(_, representation, _, _,
						_) | Alias(_, representation, _): renderType(representation, declarations, structureNames, resolving);
					case Handle(_, representation, _, _): renderType(representation, declarations, structureNames, resolving);
					case Callback(_, _, _, _, _): "RawPtr<UInt8>";
	...
	static function renderPointee(type:HxiType, declarations:Map<String, HxiDeclaration>, structureNames:Map<String, String>,
		...
		return switch type {
			case Const(element) | Nullable(element): renderPointee(element, declarations, structureNames, resolving);
			case Primitive("void"): "UInt8";
			case Named(name):
				switch declarations.get(name) {
					case Opaque(_, _) | Callback(_, _, _, _, _): "UInt8";
					case _: renderType(type, declarations, structureNames, resolving);
				};
			case _: renderType(type, declarations, structureNames, resolving);
		};
	}
