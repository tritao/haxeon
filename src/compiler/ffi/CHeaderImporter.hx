...
class CHeaderImporter {
	...
	public static function importHeader(header:String, target:String, includes:Array<String>, clang:String = "clang", ?library:String,
		...
		var handleNames:Map<String, Bool> = [], handleRecords:Map<String, Dynamic> = [];
		for (declaration in declarations) {
			var kind:String = field(declaration, "kind"), name:String = field(declaration, "name");
		...
		var output = new StringBuf(),
			...
			dependencyMetadata = dependencies == null || dependencies.length == 0 ? "" : ' @depends(' + [for (dependency in dependencies) '"$dependency"'].join(", ") + ")";
		...
		for (declaration in declarations)
			emit(declaration, layouts, output, handleNames, handleRecords);
		output.add("}\n");
	...
	static function emit(node:Dynamic, layouts:Map<String, CLayout>, output:StringBuf, handleNames:Map<String, Bool>,
			handleRecords:Map<String, Dynamic>):Void {
		var kind:String = field(node, "kind"),
			name:String = field(node, "name"),
	...
	static function validateHandle(name:String, qualified:String, layouts:Map<String, CLayout>, records:Map<String, Dynamic>):Void {
		...
		var recordName = qualified.substring(7), layout = layouts.get(recordName), record = records.get(recordName);
		...
		if (fields.length != 1 || field(fields[0], "name") != "id"
			|| mapType(field(field(fields[0], "type"), "qualType")) != "u32")
