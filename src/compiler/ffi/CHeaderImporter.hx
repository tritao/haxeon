...
class CHeaderImporter {
	...
	public static function importHeader(header:String, target:String, includes:Array<String>, clang:String = "clang", ?library:String, ?interfaceName:String,
		...
		var output = new StringBuf(),
			...
			dependencyMetadata = dependencies == null
				|| dependencies.length == 0 ? "" : ' @depends(' + [for (dependency in dependencies) '"$dependency"'].join(", ") + ")";
		output.add('// Generated by Haxeon from ${sourceLabel == null ? header : sourceLabel} for $target. Do not edit.\n');
		output.add('interface ${interfaceName == null ? moduleName(header) : interfaceName} @target("$target")$libraryMetadata$dependencyMetadata {\n');
		for (declaration in declarations)
			emit(declaration, layouts, output, handleNames, handleRecords, handleDestroySymbols);
		output.add("}\n");
	...
	static function emit(node:Dynamic, layouts:Map<String, CLayout>, output:StringBuf, handleNames:Map<String, Bool>, handleRecords:Map<String, Dynamic>,
			handleDestroySymbols:Map<String, String>):Void {
		var kind:String = field(node, "kind"),
			name:String = field(node, "name"),
		...
		switch kind {
			...
			case "TypedefDecl":
				...
				if (hasAnnotation(node, "hxi:bool32")) {
					...
					emitDocumentation(node, output);
					var destroySymbol = handleDestroySymbols.get(name),
						destroyMetadata = destroySymbol == null ? "" : ' @destroy("$destroySymbol")';
					output.add('\thandle $name : u32$destroyMetadata;\n');
			...
			case "FunctionDecl":
				...
				output.add(') -> ${borrowedUtf8 ? "utf8" : mapType(result)}${callConvention == "cdecl" ? "" : ' @callconv("' + callConvention + '")'}${borrowedUtf8 ? " @borrowed" : ""}${owned ? " @owned" : ""};\n');
		}
	}
	...
	static function parameterProjection(parameter:Dynamic):String {
		var direction = parameterDirection(parameter),
			...
			owned = hasAnnotation(parameter, "hxi:owned") ? " @owned" : "";
		if (hasAnnotation(parameter, "hxi:utf8_array")) {
			...
			return '${field(parameter, "name")}: $arrayType$direction$owned$retained';
		}
		if (hasAnnotation(parameter, "hxi:nullable_utf8"))
			return '${field(parameter, "name")}: nullable<utf8>$direction$owned$retained';
		if (hasAnnotation(parameter, "hxi:utf8"))
			return '${field(parameter, "name")}: utf8$direction$owned$retained';
		...
		return '${field(parameter, "name")}: $projected$direction$owned$retained';
	}
