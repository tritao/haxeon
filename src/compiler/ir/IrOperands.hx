...
class IrOperands {
	public static function output(instruction:IrInstruction):Null<IrValue>
		return switch instruction {
			case Phi(out, _), ConstVoid(out), ConstInt(out, _), ConstFloat(out, _), ConstString(out, _), ConstBool(out, _), ConstNull(out), TypeValue(out, _),
				ToDyn(out, _), IntToFloat(out, _), FloatToInt(out, _), SafeCast(out, _), Catch(out), GlobalGet(out, _), Add(out, _, _), Sub(out, _, _),
				Mul(out, _, _), Div(out, _, _), Mod(out, _, _), BitAnd(out, _, _), BitXor(out, _, _), BitOr(out, _, _), ShiftLeft(out, _, _),
				...
				MethodCall(out, _, _, _), NewObject(out, _), FieldGet(out, _, _), ArrayGet(out, _, _), ArraySize(out, _), MakeEnum(out, _, _, _),
				EnumIndex(out, _), EnumField(out, _, _, _): out;
	...
	public static function inputs(instruction:IrInstruction):Array<IrValue>
		return switch instruction {
			...
			case ToDyn(_, value), IntToFloat(_, value), FloatToInt(_, value), SafeCast(_, value), GlobalSet(_, value), InstanceClosure(_, _, value),
				ToVirtual(_, value), FieldGet(_, value, _), ArraySize(_, value), EnumIndex(_, value), EnumField(_, value, _, _): [value];
