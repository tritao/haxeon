...
class IrOperands {
	public static function output(instruction:IrInstruction):Null<IrValue>
		return switch instruction {
			case Phi(out, _), ConstVoid(out), ConstInt(out, _), ConstFloat(out, _), ConstString(out, _), StaticDataAddress(out, _), ConstBool(out, _),
				ConstNull(out), TypeValue(out, _), ToDyn(out, _), IntToFloat(out, _), IntToInt64(out, _), FloatToInt(out, _), SafeCast(out, _), Catch(out),
				GlobalGet(out, _), Add(out, _, _), Sub(out, _, _), Mul(out, _, _), Div(out, _, _), Mod(out, _, _), BitAnd(out, _, _), BitXor(out, _, _),
				BitOr(out, _, _), ShiftLeft(out, _, _), ShiftRight(out, _, _), UnsignedShiftRight(out, _, _), Less(out, _, _), LessEqual(out, _, _),
				Equal(out, _, _), Call(out, _, _), CNativeCall(out, _, _), StaticClosure(out, _), InstanceClosure(out, _, _), CallClosure(out, _, _),
				ToVirtual(out, _), MethodCall(out, _, _, _), NewObject(out, _), FieldGet(out, _, _), ArrayGet(out, _, _), ArraySize(out, _),
				IteratorNew(out, _), IteratorHasNext(out, _), IteratorNext(out, _), MakeEnum(out, _, _, _), EnumIndex(out, _), EnumField(out, _, _, _): out;
