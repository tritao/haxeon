...
class IrGenerator {
    ...
    public static function generateCfg(fn:TypedFunction):CfgFunction {
        ...
        var arguments = [for (argument in fn.arguments) {
            var type = lowerType(argument.type); localTypes.set(argument.name, type);
    ...
    static function lowerStatements(statements:Array<TypedStatement>, builder:CfgBuilder, localTypes:Map<String,IrType>):Void {
        for (statement in statements) {
            ...
            case TVar(name, initializer, _):
                localTypes.set(name, lowerType(initializer.type)); builder.store(name, lowerExpression(initializer,builder,localTypes));
            case TAssign(name,value,_): builder.store(name,lowerExpression(value,builder,localTypes));
            case TReturn(expression, _): builder.returnValue(lowerExpression(expression,builder,localTypes));
            case TIf(condition, thenBranch, elseBranch, _):
                var thenBlock=builder.createBlock(),elseBlock=builder.createBlock(),joinBlock=builder.createBlock();
                builder.branch(lowerExpression(condition,builder,localTypes),thenBlock,elseBlock);
                builder.select(thenBlock); lowerStatements(thenBranch,builder,localTypes); var thenActive=!builder.isTerminated(); if(thenActive)builder.jump(joinBlock);
                builder.select(elseBlock); lowerStatements(elseBranch,builder,localTypes); var elseActive=!builder.isTerminated(); if(elseActive)builder.jump(joinBlock);
                if(thenActive||elseActive) builder.select(joinBlock);
            case TWhile(condition,body,_):
                var conditionBlock=builder.createBlock(),bodyBlock=builder.createBlock(),afterBlock=builder.createBlock();
                builder.jump(conditionBlock); builder.select(conditionBlock);
                builder.branch(lowerExpression(condition,builder,localTypes),bodyBlock,afterBlock);
                builder.select(bodyBlock); lowerStatements(body,builder,localTypes); if(!builder.isTerminated())builder.jump(conditionBlock);
                builder.select(afterBlock);
            case TExpression(expression,_): lowerExpression(expression,builder,localTypes);
            }
        }
    }

    static function lowerExpression(expression:TypedExpression,builder:CfgBuilder,localTypes:Map<String,IrType>):CfgValue return switch expression.expression {
        ...
        case TLocal(name): var type=localTypes.get(name); if(type==null)throw 'Missing typed local "$name"'; builder.load(name,type);
    ...
    public static function lowerType(type:CompilerType):IrType return switch type {
        case TInt:I32; case TBool:Bool; case TFloat:F64; case TString:Bytes; case TVoid:Void;
    };
}
