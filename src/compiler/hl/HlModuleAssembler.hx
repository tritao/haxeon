...
class HlModuleAssembler {
    ...
    public function assemble(program:IrProgram, regenerated:Array<String>, signatureChanges:Array<String>):HlAssemblyResult {
        ...
        if(initialized) for(name in regenerated) {
            var index=cache.indices.get(name), stableId=cache.stableIds.get(name);
        ...
        revision++;
        initialized=true;
        return {module:module,changedFunctions:changed,changedSlots:changedSlots,requiresReload:reload,revision:revision,
