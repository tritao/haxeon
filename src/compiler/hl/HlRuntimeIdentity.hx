...
class HlRuntimeIdentity {
    ...
    public static function encodePersistent(moduleId:Bytes,stableIds:Map<String,Int>):Bytes {
        var names=[for(name in stableIds.keys())name];names.sort(Reflect.compare);
        var out=new BytesOutput();out.bigEndian=false;out.writeString("HCS");out.writeByte(1);out.write(moduleId);out.writeInt32(names.length);
        for(name in names){var bytes=Bytes.ofString(name);out.writeInt32(stableIds.get(name));out.writeInt32(bytes.length);out.write(bytes);}
        return out.getBytes();
    }
    ...
    public static function decodePersistent(bytes:Bytes):HlPersistentIdentity {
        ...
        try {
            if(input.readString(3)!="HCS"||input.readByte()!=1)throw "Invalid compiler identity state";
            var moduleId=input.read(16),count=input.readInt32(),stableIds:Map<String,Int>=[];
            if(count<0||count>0x100000)throw "Invalid compiler identity count";
            for(_ in 0...count){var id=input.readInt32(),length=input.readInt32();if(id<0||length<0||length>0x100000)throw "Invalid compiler identity entry";var name=input.readString(length);if(stableIds.exists(name))throw "Duplicate compiler identity name";stableIds.set(name,id);}
            if(input.position!=bytes.length)throw "Trailing compiler identity data";
            return {moduleId:moduleId,stableIds:stableIds};
        } catch(error:haxe.io.Eof) {throw "Truncated compiler identity state";}
    }
}
