...
class ProfilerService {
	...
	function connect(options:Dynamic):Dynamic {
		if (session != null)
			session.close();
		session = null;
		var host = stringOption(options, "host", "127.0.0.1"),
			port = requiredInt(options, "port"),
			timeout = floatOption(options, "timeoutSeconds", 5.0),
	...
	function start(options:Dynamic):Dynamic {
		var current = requireSession(),
			rate = intOption(options, "sampleRate", 1000),
			interval = intOption(options, "pollIntervalMs", 100),
		...
		if (allocationInterval < 0)
			throw "Profiler allocation interval must not be negative";
	...
	function captureStart(options:Dynamic):Dynamic {
		...
		if (path.length == 0)
			throw "Profiler capture path must not be empty";
	...
	function snapshot(value:ProfilerSnapshot, incremental:Bool = false):Dynamic {
		...
		var timelineReset = emittedTimelineSequence == 0,
			timelineGap = !timelineReset && oldestSequence > emittedTimelineSequence + 1;
		if (timelineGap) {
			emittedTimelineStacks.clear();
			timelineEpoch++;
		}
		var newSamples = [
			for (sample in value.timelineSamples)
				if (timelineReset || timelineGap || sample.sequence > emittedTimelineSequence) sample
		];
		var requiredStacks = new Map<String, Bool>();
		for (sample in newSamples)
			if (!emittedTimelineStacks.exists(sample.stackKey))
				requiredStacks.set(sample.stackKey, true);
		var newStacks = [
			for (stack in value.stacks)
				if (requiredStacks.exists(stack.key)) stackValue(stack)
		];
		for (stack in newStacks)
			emittedTimelineStacks.set(stack.key, true);
		if (newSamples.length > 0)
			emittedTimelineSequence = newSamples[newSamples.length - 1].sequence;
		var newGcStats = [
			for (stats in value.gcStats)
				if (timelineReset || timelineGap || stats.timestamp > emittedGcTimestamp) stats
		];
		if (newGcStats.length > 0)
			emittedGcTimestamp = newGcStats[newGcStats.length - 1].timestamp;
		var newAllocations = [
			for (sample in value.allocationSamples)
				if (timelineReset || timelineGap || sample.sequence > emittedAllocationSequence) sample
		];
		if (newAllocations.length > 0)
			emittedAllocationSequence = newAllocations[newAllocations.length - 1].sequence;
		var result:Dynamic = {
			...
			threads: [
				for (threadId => name in value.threads)
					{
						id: threadId,
						name: name
					}
			...
			timeline: {
				epoch: timelineEpoch,
				reset: timelineReset || timelineGap,
				gap: timelineGap,
				fromSequence: newSamples.length == 0 ? emittedTimelineSequence : newSamples[0].sequence,
				toSequence: emittedTimelineSequence,
				samples: [
					for (sample in newSamples)
						{
							...
							stackKey: sample.stackKey
						}
				],
				stacks: newStacks,
				counters: downsampleGc(newGcStats, 128),
				allocations: [
					for (sample in newAllocations)
						{
							sequence: sample.sequence,
							timestamp: sample.timestamp,
							threadId: sample.threadId,
			...
			metadataRevisions: [
				for (moduleId => revision in value.metadataRevisions)
					{moduleId: moduleId, revision: revision}
			],
			metadataChanges: [
				for (change in value.metadataChanges)
					{
						timestamp: change.timestamp,
						moduleId: change.moduleId,
						oldRevision: change.oldRevision,
			...
			functions: [
				for (aggregate in value.functions.slice(0, maxEntries))
	...
	static function downsampleGc(values:Array<profiler.ProfilerSession.ProfileGcStats>, limit:Int):Array<Dynamic> {
		...
		var result:Array<Dynamic> = [],
			step = (values.length - 1) / (limit - 1);
		for (index in 0...limit)
			result.push(gcStatsValue(values[Math.round(index * step)]));
		return result;
	}
	...
	static function stackValue(value:ProfileStack):Dynamic
		return {
			...
			threadSamples: [
				for (threadId => samples in value.threadSamples)
					{
						threadId: threadId,
						samples: samples
					}
	...
	static function frameValue(frame:profiler.ProfilerSession.ProfileStackFrame):Dynamic
		return {
			...
			file: frame.file,
			line: frame.line,
			nativeModule: frame.nativeModule,
