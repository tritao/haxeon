...
class FfiAuditMain {
	static function main():Void {
		var targets:Array<String> = [], includes:Array<String> = [], paths:Array<String> = [], format = "text", profile:Null<String> = null,
			output:Null<String> = null, library:Null<String> = null, interfaceName:Null<String> = null, sourceLabel:Null<String> = null,
		...
		for (argument in Sys.args())
			...
			else if (StringTools.startsWith(argument, "--source-label=")) sourceLabel = argument.substring(15);
			else if (StringTools.startsWith(argument, "--exclude-header=")) excludedHeaders.push(argument.substring(17));
		...
		if (paths.length != 1 || targets.length < 2 || (format != "text" && format != "json"))
			throw "Usage: haxeon-ffi-audit --target=<triple> --target=<triple> [--profile=portable-abi64] [--format=text|json] [--output=<file>] [--library=<name>] [--interface=<name>] [--include=<dir>] [--source-label=<path>] [--exclude-header=<path>] <header>";
		var report = HxiAudit.audit(paths[0], targets, includes, profile, library, interfaceName, excludedHeaders);
		...
		if (output != null) {
			...
			File.saveContent(temporary, HxiAudit.canonical(paths[0], targets[0], includes, library, interfaceName, sourceLabel, excludedHeaders));
