...
class FfiAuditMain {
	static function main():Void {
		...
		if (paths.length != 1 || targets.length < 2 || (format != "text" && format != "json"))
			throw "Usage: haxeon-ffi-audit --target=<triple> --target=<triple> [--profile=portable-abi64] [--format=text|json] [--output=<file>] [--library=<name>] [--interface=<name>] [--include=<dir>] [--source-label=<path>] <header>";
		...
		if (output != null) {
			...
			File.saveContent(temporary, HxiAudit.canonical(paths[0], targets[0], includes, library, interfaceName, sourceLabel));
