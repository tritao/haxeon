...
class FfiAuditMain {
	static function main():Void {
		var targets:Array<String> = [],
			...
			library:Null<String> = null,
			interfaceName:Null<String> = null,
			sourceLabel:Null<String> = null,
			dependencies:Array<String> = [],
			excludedHeaders:Array<String> = [],
			dependencyPaths:Array<String> = [];
		for (argument in Sys.args())
			if (StringTools.startsWith(argument, "--target="))
				targets.push(argument.substring(9));
			else if (StringTools.startsWith(argument, "--include="))
				includes.push(argument.substring(10));
			else if (StringTools.startsWith(argument, "--format="))
				format = argument.substring(9);
			else if (StringTools.startsWith(argument, "--profile="))
				profile = argument.substring(10);
			else if (StringTools.startsWith(argument, "--output="))
				output = argument.substring(9);
			else if (StringTools.startsWith(argument, "--library="))
				library = argument.substring(10);
			else if (StringTools.startsWith(argument, "--interface="))
				interfaceName = argument.substring(12);
			else if (StringTools.startsWith(argument, "--depends="))
				dependencies.push(argument.substring(10));
			else if (StringTools.startsWith(argument, "--dependency-hxi="))
				dependencyPaths.push(argument.substring(17));
			else if (StringTools.startsWith(argument, "--source-label="))
				sourceLabel = argument.substring(15);
			else if (StringTools.startsWith(argument, "--exclude-header="))
				excludedHeaders.push(argument.substring(17));
			else if (StringTools.startsWith(argument, "--"))
				throw 'Unknown FFI audit option "$argument"';
			else
				paths.push(argument);
		...
		if (format == "json")
			...
			for (issue in report.issues)
				Sys.println('  ${issue.target} [${issue.kind}]: ${issue.message}');
		}
		if (!report.identical)
			Sys.exit(1);
		if (output != null) {
