...
export class ProfilerPanel {
  ...
  static show(extensionUri: vscode.Uri, client: LanguageClient): void {
    if (this.current) {
      this.current.panel.reveal();
      return;
    }
    this.current = new ProfilerPanel(extensionUri, client);
  }

  private constructor(extensionUri: vscode.Uri, private readonly client: LanguageClient) {
    ...
    this.disposables.push(
      client.onNotification("haxeon/profilerSnapshot", value => this.panel.webview.postMessage({type: "snapshot", value})),
  ...
  private async receive(message: Message): Promise<void> {
    ...
    const options = command === "connect" ? {host: config.get("host"), port: config.get("port")}
      : command === "start" ? {sampleRate: Number(message.sampleRate ?? config.get("sampleRate")), pollIntervalMs: 100} : {};
    try {
...
function html(webview: vscode.Webview): string {
  ...
  <div id="health" class="health"></div><div id="markers"></div><div id="error"></div><div class="layout"><section><h3>Call tree</h3><div id="tree"></div></section><section><h3>Flame graph</h3><div id="flame"></div></section></div>
