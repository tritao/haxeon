...
function updateHealth(h){if(!h)return;document.getElementById('health').innerHTML='<span>buffer '+Math.round((h.bufferUtilization||0)*100)+'%</span><span>rate '+h.effectiveSampleRate+'/'+h.requestedSampleRate+' Hz</span><span>dropped '+h.dropped+'</span><span>GC '+(h.gcSamples||0)+'</span><span>overhead '+Number(h.overheadMicrosPerSample||0).toFixed(1)+' µs/sample</span><span>generated '+(h.generatedBytes||0)+' B</span><span>metadata '+Number(h.metadataRefreshMs||0).toFixed(1)+' ms</span>';const s=document.getElementById('thread'),selected=s.value;(h.threads||[]).forEach(t=>{if(![...s.options].some(o=>o.value==String(t.id)))s.add(new Option(t.name,String(t.id)))});s.value=[...s.options].some(o=>o.value===selected)?selected:'all'}
render();`;
}
