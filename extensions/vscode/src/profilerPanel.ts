...
window.addEventListener('message',({data})=>{if(data.type==='error'){document.getElementById('error').textContent=data.value;return}if(data.type==='recording'){document.getElementById('capture').textContent=data.active?'Stop recording':'Record';return}if(data.type==='offline'||data.type==='comparison'){loadCaptures(data.captures);return}if(data.type==='connection'){document.getElementById('connection').textContent=data.state+(data.delay?' in '+data.delay+'ms':'')+(data.gap?' — sample gap after reconnect':'');return}if(data.type==='revision')markers.push(data.value);if(data.type==='snapshot'){const v=data.value||{};if((v.samples||0)<samples){nodes.clear();stackDefinitions.clear();markers.length=0;selectedRange=null}samples=v.samples||0;if(nodes.size===0&&v.view?.flameGraph){v.view.flameGraph.forEach(n=>nodes.set(n.id,n));markers.splice(0,markers.length,...(v.view.revisionMarkers||[]))}else if(v.viewDelta?.nodes)v.viewDelta.nodes.forEach(n=>nodes.set(n.id,n));(v.timelineStacks||v.stacks||[]).forEach(s=>stackDefinitions.set(s.key,s.frameDetails));timelineSamples=v.timelineSamples||[];gcTimeline=v.gcStats||[];if(selectedRange)rebuildRange();updateHealth(v.viewDelta?.health||v.view?.health);}(data.value?.view?.revisionMarkers||[]).forEach(addRevision);if(data.value)addRevision(data.value);render()});
...
function updateHealth(h){if(!h)return;const gc=(h.gcStats||[]).at(-1),memory=gc?'<span>heap '+formatBytes(gc.heap)+'</span><span>allocated '+formatBytes(gc.allocated)+'</span><span>collections '+gc.collections+'</span>':'';document.getElementById('health').innerHTML='<span>buffer '+Math.round((h.bufferUtilization||0)*100)+'%</span><span>rate '+h.effectiveSampleRate+'/'+h.requestedSampleRate+' Hz</span><span>dropped '+h.dropped+'</span><span>GC samples '+(h.gcSamples||0)+'</span>'+memory+'<span>native symbols '+(h.nativeSymbolCount||0)+'</span><span>overhead '+Number(h.overheadMicrosPerSample||0).toFixed(1)+' µs/sample</span><span>generated '+(h.generatedBytes||0)+' B</span><span>metadata '+Number(h.metadataRefreshMs||0).toFixed(1)+' ms</span>';const s=document.getElementById('thread'),selected=s.value;(h.threads||[]).forEach(t=>{if(![...s.options].some(o=>o.value==String(t.id)))s.add(new Option(t.name,String(t.id)))});s.value=[...s.options].some(o=>o.value===selected)?selected:'all'}
