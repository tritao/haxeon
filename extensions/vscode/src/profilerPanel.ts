...
export class ProfilerPanel {
  ...
  private async receive(message: Message): Promise<void> {
    ...
    if (message.type === "exportCapture") {
      ...
      const start = Number(message.startMs), end = Number(message.endMs);
      if (Number.isFinite(start) && Number.isFinite(end) && end > start) args.push("--start-ms", String(Math.max(0, Math.floor(start))), "--end-ms", String(Math.ceil(end)));
	...
	if (command === "start") { this.desiredConnected = true; this.desiredRunning = true; this.sampleRate = Number(message.sampleRate ?? 250); this.allocationInterval = Number(message.allocationInterval ?? 0); }
  ...
  private async execute(command: string, message: Message = {type: "command"}): Promise<boolean> {
    ...
    const options = command === "connect" ? {host: config.get("host"), port: config.get("port"), token}
      : command === "start" ? {sampleRate: Number(message.sampleRate ?? this.sampleRate ?? config.get("sampleRate")), allocationInterval: Number(message.allocationInterval ?? this.allocationInterval), pollIntervalMs: 100} : {};
  ...
  private scheduleReconnect(): void {
    ...
    this.reconnectTimer = setTimeout(async () => {
      ...
      if (await this.execute("connect") && (!this.desiredRunning || await this.execute("start", {type: "command", sampleRate: this.sampleRate, allocationInterval: this.allocationInterval}))) {
...
function html(webview: vscode.Webview): string {
  ...
  <div class="toolbar"><button data-command="connect">Connect</button><button data-command="start">Start</button><button data-command="pause">Pause</button><button data-command="reset">Reset</button><button id="capture">Record</button><button id="openCapture">Open</button><button id="compareCapture">Compare</button><button data-export="folded">Export folded</button><button data-export="perfetto">Export Perfetto</button><label>CPU Hz <input id="rate" type="number" min="1" max="100000" value="250"></label><label>sample 1/N allocations <input id="allocationInterval" type="number" min="0" max="100000000" value="0"></label><label>range ms <input id="startMs" type="number" min="0" value="0"></label><label>to <input id="endMs" type="number" min="0" value="9007199254740991"></label><select id="mode"><option value="cpu">CPU samples</option><option value="alloc">Allocated bytes</option></select><select id="thread"><option value="all">All threads</option></select><select id="revision"><option value="all">All revisions</option></select></div>
...
function script(): string {
  return `const vscode=acquireVsCodeApi(),nodes=new Map(),filteredNodes=new Map(),markers=[],stackDefinitions=new Map();let samples=0,timelineSamples=[],gcTimeline=[],allocationSamples=[],selectedRange=null,dragStart=null;
document.querySelectorAll('[data-command]').forEach(b=>b.onclick=()=>vscode.postMessage({type:'command',command:b.dataset.command,sampleRate:Number(document.getElementById('rate').value),allocationInterval:Number(document.getElementById('allocationInterval').value)}));
const range=()=>({startMs:Number(document.getElementById('startMs').value),endMs:Number(document.getElementById('endMs').value)}),selectedExportRange=()=>{if(!selectedRange)return{};const b=timeBounds();return{startMs:(selectedRange[0]-b[0])*1000,endMs:(selectedRange[1]-b[0])*1000}};document.getElementById('capture').onclick=()=>vscode.postMessage({type:'capture'});document.getElementById('openCapture').onclick=()=>vscode.postMessage({type:'openCapture',...range()});document.getElementById('compareCapture').onclick=()=>vscode.postMessage({type:'compareCapture',...range()});document.querySelectorAll('[data-export]').forEach(b=>b.onclick=()=>vscode.postMessage({type:'exportCapture',format:b.dataset.export,...selectedExportRange()}));
...
function render(){const started=performance.now(),allocationMode=document.getElementById('mode').value==='alloc',source=allocationMode?allocationNodes():selectedRange?filteredNodes:nodes,values=[...source.values()].filter(visible),shownSamples=[...source.values()].filter(n=>n.depth===0).reduce((n,v)=>n+v.totalSamples,0)||samples,tree=document.getElementById('tree');tree.textContent='';values.forEach(n=>{const d=document.createElement('div');d.className='tree-row '+(n.delta>0?'regression':n.delta<0?'improvement':'');d.style.paddingLeft=(n.depth*16)+'px';d.textContent=n.name+'  '+formatMetric(n.selfSamples,allocationMode)+'/'+formatMetric(n.totalSamples,allocationMode)+(n.delta?' Δ'+n.delta:'');d.title=((n.file||'')+(n.line?':'+n.line:''))+(n.delta?' comparison delta '+n.delta:'');d.onclick=()=>open(n);tree.appendChild(d)});const flame=document.getElementById('flame');flame.textContent='';const depths=Math.max(0,...values.map(n=>n.depth));for(let depth=0;depth<=depths;depth++){const row=document.createElement('div');row.className='flame-row';values.filter(n=>n.depth===depth).forEach(n=>{const d=document.createElement('div');d.className='flame';d.style.width=Math.max(1,n.totalSamples*100/Math.max(1,shownSamples))+'%';d.textContent=n.name+' '+formatMetric(n.totalSamples,allocationMode);d.onclick=()=>open(n);row.appendChild(d)});flame.appendChild(row)}document.getElementById('markers').innerHTML=markers.map(m=>'<div class="marker">↻ module '+m.moduleId+' revision '+m.oldRevision+' → '+m.newRevision+'</div>').join('');renderTimeline();const latency=document.getElementById('uiLatency');if(latency)latency.textContent='UI '+(performance.now()-started).toFixed(1)+' ms'}

function allocationNodes(){const result=new Map(),thread=document.getElementById('thread').value;allocationSamples.filter(s=>(!selectedRange||(s.timestamp>=selectedRange[0]&&s.timestamp<=selectedRange[1]))&&(thread==='all'||s.threadId===Number(thread))).forEach(s=>{let parent='',depth=0;const weight=Number(s.allocated)*s.interval;(s.frameDetails||[]).forEach(frame=>{const id=parent?parent+'>'+frame.stableKey:frame.stableKey,n=result.get(id)||{...frame,id,parentId:parent||null,depth,selfSamples:0,totalSamples:0,revisions:[frame.revision],threadSamples:[{threadId:s.threadId,samples:0}]};n.totalSamples+=weight;result.set(id,n);parent=id;depth++});if(parent)result.get(parent).selfSamples+=weight});return result}
function formatMetric(value,bytes){return bytes?formatBytes(value):value}
