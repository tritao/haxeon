...
for case_name in add array-iterator-wasm array-slice-index array-growth-wasm dynamic-equality numeric-promotion function-wrapper try-catch try-nested try-array-bounds; do
	"$haxe_bin" --cwd "$root_dir" -cp src --run compiler.tools.HaxeonCompiler \
