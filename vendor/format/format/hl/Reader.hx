...
class Reader {
	...
	function readFunction() : HLFunction {
		...
		return {
			...
			assigns : flags.has(HasDebug) && version >= 3 ? [for( i in 0...uindex() ) { varName : uindex(), position : index() - 1, scopeEnd : version >= 6 ? index() - 1 : -1 }] : [],
		};
	}
	...
	public function read( i : haxe.io.Input ) : Data {
		...
		for( i in 0...ntypes )
			...
			case HObj(p), HStruct(p):
				if( p.tsuper != null ) p.tsuper = fixType(p.tsuper);
				for( f in p.fields )
					f.t = fixType(f.t);
			case HVirtual(fl):
				for( f in fl )
