...
| Explicit Haxe GC handles | HashLink GC handle bridge | `LoadedModule` or `HlRuntimeModule` for owned handles; process runtime for unowned handles | Haxeon/native consumers that explicitly retain the handle | Owned handles close during module retirement; unowned handles close explicitly or when finalized |
